...
will spam left clicks. Works by listening to events from `/dev/input` to trigger other
events into `uinput`. The lib `uinput` should already be setup with Wayland.
